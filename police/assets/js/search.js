...
function exactTrigger(d,q){const nq=norm(q);return (d.triggers||[]).some(t=>norm(t)===nq)}
...
const parts=q.split('、').map(x=>x.trim()).filter(Boolean);if(parts.length>2){status.textContent='3語以上の複合検索はできません。';return}if(q.includes('、')&&parts.length!==2){status.textContent='複合検索は2つの検索ワードを「、」で区切ってください。';return}
